...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    public void scrollTo(int x, int y) {
        ...
        if (LauncherLog.DEBUG_DRAW) {
            LauncherLog.d(TAG, "scrollTo: x = " + x + ", y = " + y + ", mOverScrollX = "
                + mMaxScrollX + ", mScrollX = " + getScrollX() + ", this = " + this);
        }

        boolean isXBeforeFirstPage = mIsRtl ? (x > mMaxScrollX) : (x < 0);
        boolean isXAfterLastPage = mIsRtl ? (x < 0) : (x > mMaxScrollX);
        if (LauncherLog.DEBUG_DRAW) {
            LauncherLog.d(TAG, "scrollTo: x = " + x + ", y = " + y + ", mMaxScrollX = "
                    + mMaxScrollX + ", mScrollX = " + getScrollX()
                    + ", mIsRtl = " + mIsRtl + ", isXBeforeFirstPage = " + isXBeforeFirstPage
                    + ", isXAfterLastPage = " + isXAfterLastPage + ", this = " + this);
        }
        if (isXBeforeFirstPage) {
            /// M: ALPS02295402, patch ALPS02195756,In rtl environment,
            /// if drag icon and let it go at first page, will run into below case.
            ...
            /// M: ALPS02295402, add patch ALPS02195756,
    ...
    public void removeView(View v) {
        ...
        ///M, ALPS02295402, add patch ALPS02195756,,set mRemovedEmptyPage is true.
        if (mIsRtl && mCurrentPage == 0) {
        ...
        if (pageCount > 0) {
            ...
            if (LauncherLog.DEBUG_DRAW) {
                LauncherLog.d(TAG, "dispatchDraw: mScrollX = " + getScrollX()
                    ...
                    + mMaxScrollX + ", mLastScreenCenter = " + mLastScreenCenter + ", mLeft = "
                    + getLeft() + ", mRight = " + getRight() + ",mForceScreenScrolled = "
                    + mForceScreenScrolled + ",getWidth() = " + getWidth() + ", pageCount = "
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mActivePointerId != INVALID_POINTER) {
                    ...
                    /// M: [Performance] Scroll page in first performTraversal.
                    //  to improve response time Scroll to follow the motion event {@
                    ...
                    if (mTouchState == TOUCH_STATE_SCROLLING) {
                        ...
                        // Only scroll and update mLastMotionX if
                        // we have moved some discrete amount.  We
                        // keep the remainder because we are actually
                        // testing if we've moved from the last
                        ...
                        if (LauncherLog.DEBUG) {
                            LauncherLog.d(TAG, "onInterceptTouchEvent, Touch move scroll: x = "
                                    + x + ", deltaX = " + deltaX + ", mTotalMotionX = "
                                    + mTotalMotionX + ", mLastMotionX = " + mLastMotionX
                                    + ", mCurrentPage = " + mCurrentPage + ",mTouchX = "
                                    + mTouchX + " ,mLastMotionX = " + mLastMotionX
                                    + ", mScrollX = " + getScrollX());
                        }
                        //@}
                    }
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                if (LauncherLog.DEBUG) {
                    LauncherLog.d(TAG, "onInterceptTouchEvent touch down: finishedScrolling = "
                            + finishedScrolling + ", mScrollX = " + getScrollX()
                            + ", xDist = " + xDist + ", mTouchState = " + mTouchState
                            + ", this = " + this);
                }
                break;
            }
         ...
         */
        if (LauncherLog.DEBUG_MOTION) {
            LauncherLog.d(TAG, "onInterceptTouchEvent: return = "
                + (mTouchState != TOUCH_STATE_REST));
        }
    ...
    protected float getScrollProgress(int screenCenter, View v, int page) {
        ...
        if (LauncherLog.DEBUG_DRAW) {
            LauncherLog.d(TAG, "getScrollProgress: screenCenter = " + screenCenter
                    + ", page = " + page + ", v = " + v + ",totalDistance = "
                    + totalDistance + ", mPageSpacing = " + mPageSpacing
                    + ", delta = " + delta + ", halfScreenSize = "
                    + halfScreenSize + ", scrollProgress = " + scrollProgress);
        }
            ...
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (LauncherLog.DEBUG) {
                    LauncherLog.d(TAG, "Touch move scroll: x = " + x + ", deltaX = " + deltaX
                            ...
                            + mTouchX + " ,mLastMotionX = " + mLastMotionX + ", mScrollX = "
                            + getScrollX());
                }
    ...
    protected void snapToPageWithVelocity(int whichPage, int velocity) {
        ...
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "snapToPageWithVelocity: velocity = " + velocity
    ...
    protected void snapToPage(int whichPage, int delta, int duration, boolean immediate,
        ...
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "(PagedView)snapToPage whichPage = " + whichPage + ", delta = "
                ...
                + mDeferScrollUpdate +  ",mOverScrollX = " + mOverScrollX + */", mScrollX = "
                /*+ mScrollX*/ + ", this = " + this);
    ...
    void endReordering() {
        ...
        /// M: ALPS02431679, will have overlay shortcut
        mDragView = null;
    }
