...
public class LauncherModelPluginEx {
     ...
     */
    public static void addAllAppsItemToDatabase(Context context, final ItemInfo item,
        ...
        //RANK is no need.
        values.remove(LauncherSettings.Favorites.RANK);
        values.remove(LauncherSettings.Favorites.OPTIONS);

        item.id = LauncherExtPlugin.getInstance().getLoadDataExt(context)
            .generateNewIdForAllAppsList();
        values.put(AllApps._ID, item.id);
        values.put(AllApps.TITLE, item.title.toString());
        ...
        if (item instanceof AppInfo) {
             values.put(AllApps.INTENT, ((AppInfo) item).intent.toUri(0));
        }
   ...
   public static void addFolderItemToDatabase(Context context, final FolderInfo item,
           final int screen, final int cellX, final int cellY, final boolean notify) {
       if (LauncherLog.DEBUG) {
   ...
   /**
     ...
     */
    public static void updateAllAppsItemInDatabase(Context context, final ItemInfo item) {
        if (LauncherLog.DEBUG) {
     ...
     */
    public static void updateAllAppsItemInDatabaseHelper(Context context,
     ...
     */
    public static void moveAllAppsItemInDatabase(Context context, final ItemInfo item,
   ...
   /**
     ...
     * @param item
     */
    public static void deleteAllAppsItemFromDatabase(Context context, final ItemInfo item) {
        if (LauncherLog.DEBUG) {
    ...
    public static void deleteFolderItemFromDatabase(Context context, final FolderInfo item) {
        if (LauncherLog.DEBUG) {
    ...
    */
            ...
            mLauncherModel.mHandler.post(new Runnable() {
                public void run() {
                    ...
                    if (callbacks != null) {
                        ...
                        mLauncherModel.loadAndBindWidgetsAndShortcuts(callbacks,
                                true /* refresh */);
                        //callbacks.bindAllApplications(allApps);
                        if (DEBUG_LOADERS) {
    ...
    public void onlyBindAllAppsExt() {
        final Callbacks oldCallbacks = mLauncherModel.getCallback();
        if (oldCallbacks == null) {
        ...
        final WidgetsModel widgetList = mLauncherModel.mBgWidgetsModel.clone();
        final Runnable r = new Runnable() {
            public void run() {
                final long t = SystemClock.uptimeMillis();
                ...
                if (callbacks != null) {
                    ...
                    callbacks.refreshAppsView(false);
    ...
    public void loadAllAppsExt(UserHandleCompat user, final ArrayList<ItemInfo> allItems,
        ...
        final LongArrayMap<FolderInfo> appFolders = new LongArrayMap<>();
                            ...
                            if (container < 0) {
                                ...
                                if (LauncherLog.DEBUG_LOADER) {
                                    LauncherLog.d(TAG, "loadAllApps add: "
                                            ...
                                            + appFolders.size()
                                            + ",folderInfo @"
                                            + Integer.toHexString(folderInfo.hashCode()));
                                }
                            }
        ...
        if (loadDefault) {
            AppNameComparator appNameComparator = new AppNameComparator(mContext);
            //Collections.sort(allApps, appNameComparator.getAppInfoComparator());
            for (int i = 0; i < allApps.size(); i++) {
                AppInfo tempinfo = allApps.get(i);
                tempinfo.screenId = 0;
                tempinfo.cellX = i;
                tempinfo.cellY = 0;
                moveAllAppsItemInDatabase(context, tempinfo, (int) tempinfo.screenId,
                    tempinfo.cellX, tempinfo.cellY);
            }
        }

        for (AppInfo tempInfo : allApps) {
            tempInfo.unreadNum = MTKUnreadLoader.getUnreadNumberOfComponent(
                tempInfo.componentName);
            if (LauncherLog.DEBUG_UNREAD) {
                LauncherLog.d(TAG, "loadAllAppsExt: appInfo.componentName = "
                    + tempInfo.componentName
                    + ", appInfo.unreadNum = " + tempInfo.unreadNum);
            }
        }
        for (FolderInfo tempInfo : allFolders) {
            for (ShortcutInfo shortcutInfo : tempInfo.contents) {
                shortcutInfo.unreadNum = MTKUnreadLoader.getUnreadNumberOfComponent(
                    shortcutInfo.mComponentName);
                if (LauncherLog.DEBUG_UNREAD) {
                    LauncherLog.d(TAG, "loadAllAppsExt: shortcutInfo.componentName = "
                        + shortcutInfo.mComponentName
                        + ", shortcutInfo.unreadNum = " + shortcutInfo.unreadNum);
                }
                tempInfo.unreadNum += shortcutInfo.unreadNum;
            }
            if (LauncherLog.DEBUG_UNREAD) {
                LauncherLog.d(TAG, "loadAllAppsExt: folderInfo.title = "
                    + tempInfo.title
                    + ", folderInfo.unreadNum = " + tempInfo.unreadNum);
            }
        }
