...
public class AllAppsListPluginEx { //Modify private to public BUG_ID:DWYQLSSMY-373 zhaopenglin 20160426
   ...
   static void ensureTopPackageOrdered() {
    ...
    public static void reorderApplist(List<AppInfo> mApps) {
        final long sortTime = DEBUG_LOADERS_REORDER ? SystemClock.uptimeMillis() : 0;
        ...
        final ArrayList<AppInfo> dataReorder = new ArrayList<AppInfo>(8);
        ...
        int newIndex = 0;
        for (AppInfo ai : dataReorder) {
            newIndex = Math.min(newIndex, mApps.size());
            if (DEBUG_LOADERS_REORDER) {
                LauncherLog.d(TAG, "reorderApplist: added newIndex = " + newIndex);
            }
            /// M: make sure the array list not out of bound
            if (newIndex < mApps.size()) {
    ...
    //Modify BUG_ID:DWYQLSSMY-373 zhaopenglin 20160426(eng)
